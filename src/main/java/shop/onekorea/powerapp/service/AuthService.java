...
import shop.onekorea.powerapp.dto.SignInRequestDto;
import shop.onekorea.powerapp.dto.SignInResponseDto;
...
import shop.onekorea.powerapp.security.TokenProvider;
...
//            userEntity = userRepository.findByEmail(email);
            ...
            if (!passwordEncoder.matches(password, userEntity.getPassword()))
                return ResponseDto.setFailed("비빌 번호가 맞지 않네요. 비밀 번호를 다시 확인하시오!");
        } catch (Exception e) {
...
//        String token = tokenProvider.generateToken(email);//        int expiration = 60 * 60 * 24 * 7; //
        String token = tokenProvider.generateToken(userid);//        int expiration = 60 * 60 * 24 * 7; //
...
import org.springframework.stereotype.Service;
import org.springframework.web.bind.annotation.PostMapping;
import shop.onekorea.powerapp.dto.*;
import shop.onekorea.powerapp.entity.UserEntity;
import shop.onekorea.powerapp.repository.UserRepository;
import shop.onekorea.powerapp.security.TokenProvider;
...
//            if (userRepository.existsById(email))
        ...
        try {
            System.out.println("=====> userEntity: " + userEntity.toString());
            userRepository.save(userEntity);
            return ResponseDto.setSuccess("Sign Up Success!!!", userEntity); // SignUp.회원 가입에서는 아직 "token"이 없다.
//            return ResponseDto.setSuccessNoToken("Sign Up Success 회원 가입 성공!!!", userEntity); // SignUp.회원 가입에서는 아직 "token"이 없다.
...
//    public ResponseDto<?> signIn(SignInRequestDto requestDto) {
    public ResponseDto<SignInResponseDto> signIn(SignInRequestDto requestDto) {
        System.out.println("=====> AuthService.serviceSignIn.requestDto: " + requestDto.toString());
...
//            userEntity = userRepository.findByEmail(email);
            userEntity = userRepository.findByUserid(userid);
            // email check
            if (userEntity == null) return ResponseDto.setFailed("등록된 이메일이 없네요. 이메일을 다시 확인하시오!");
            // password check
            if (!passwordEncoder.matches(password, userEntity.getPassword()))
                return ResponseDto.setFailed("비빌 번호가 맞지 않네요. 비밀 번호를 다시 확인하시오!");
        } catch (Exception e) {
...
//        }
        ...
        System.out.println("=====> AuthService.serviceSignIn.userEntity:password 공란: " + userEntity.toString());
...
//        int expiration = 60 * 60 * 24 * 30; //
        ...
        SignInResponseDto signInResponseDto = new SignInResponseDto(token, expiration, userEntity);
        System.out.println("=====> AuthService.signIn.signInResponseDto: " + signInResponseDto.toString());
        System.out.println("=====> AuthService.signIn.token: " + token);
